...
if [ -n "$LINT_FILES" ]; then
  ...
  # Check for TypeScript dependencies if TS files are staged
  NEEDS_TS_DEPS=0
  ...
  if [ -z "$ESLINT_BIN" ] || [ $ESLINT_JS_MISSING -eq 1 ] || [ $TS_DEPS_MISSING -eq 1 ]; then
    echo "[ESLint] Missing dependencies — attempting automatic installation..."
    INSTALL_CMD="npm install --save-dev eslint @eslint/js eslint-plugin-import-x eslint-config-prettier --legacy-peer-deps"
    if [ $NEEDS_TS_DEPS -eq 1 ]; then
      INSTALL_CMD="$INSTALL_CMD @typescript-eslint/parser @typescript-eslint/eslint-plugin typescript eslint-import-resolver-typescript"
    fi
    $INSTALL_CMD --quiet 2>&1 | tail -n 3
    if [ -f "./node_modules/.bin/eslint" ]; then
      ESLINT_BIN="./node_modules/.bin/eslint"
    fi
  fi
