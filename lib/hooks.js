...
if [ -n "$LINT_FILES" ]; then
  ...
  # Check if @eslint/js is missing when needed
  ESLINT_JS_MISSING=0
  if [ $NEEDS_ESLINT_JS -eq 1 ] && [ ! -d "./node_modules/@eslint/js" ]; then
    ESLINT_JS_MISSING=1
  fi

  # Check for TypeScript dependencies if TS files are staged OR if project is TS
  NEEDS_TS_DEPS=0
  if echo "$LINT_FILES" | grep -qE "\\.(ts|tsx)$"; then
    NEEDS_TS_DEPS=1
  elif [ -f "tsconfig.json" ] || [ -n "$(find . -maxdepth 2 -name "*.ts" -o -name "*.tsx" 2>/dev/null | head -n 1)" ]; then
    NEEDS_TS_DEPS=1
  fi

  TS_DEPS_MISSING=0
  if [ $NEEDS_TS_DEPS -eq 1 ] && { [ ! -d "./node_modules/@typescript-eslint/parser" ] || [ ! -d "./node_modules/@typescript-eslint/eslint-plugin" ]; }; then
    TS_DEPS_MISSING=1
  fi

  if [ -z "$ESLINT_BIN" ] || [ $ESLINT_JS_MISSING -eq 1 ] || [ $TS_DEPS_MISSING -eq 1 ]; then
    echo "[ESLint] Missing dependencies — attempting automatic installation (this may take a moment)..."
    INSTALL_CMD="npm install --save-dev eslint @eslint/js --legacy-peer-deps --no-audit --no-fund"
    if [ $NEEDS_TS_DEPS -eq 1 ]; then
      INSTALL_CMD="$INSTALL_CMD @typescript-eslint/parser @typescript-eslint/eslint-plugin typescript"
    fi
    $INSTALL_CMD
    if [ -f "./node_modules/.bin/eslint" ]; then
      ...
      echo "❌ [ESLint] Automatic installation failed. Run 'npm install' manually."
      exit 1
    fi
  fi
  ...
  if [ -n "$ESLINT_BIN" ]; then
    ...
    if [ $HAS_CONFIG -eq 1 ]; then
      echo "[ESLint] Running full project lint check..."
      if ! "$ESLINT_BIN" .; then
        echo ""
