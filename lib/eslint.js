...
exports.setupESLintConfig = async () => {
  ...
  if (hasConfig) {
    ...
    } else if (existingConfigFile === 'eslint.config.mjs') {
      // NEW: Intelligent update for flat config
      const targetPath = path.join(projectRoot, 'eslint.config.mjs');
      const configContent = await fs.readFile(targetPath, 'utf8');
      
      if (!configContent.includes('tsFallback') && !configContent.includes('**/*.ts')) {
        logInfo('Outdated eslint.config.mjs detected (missing TS patterns) — updating to latest template...');
        ...
        logSuccess('eslint.config.mjs updated with TypeScript fallback support.');
      } else {
        logInfo('Existing eslint.config.mjs already contains required patterns — skipping update.');
      }
    } else {
      logInfo(`Existing config is ${existingConfigFile} — skipping merge (not .eslintrc.json or eslint.config.mjs).`);
    }
